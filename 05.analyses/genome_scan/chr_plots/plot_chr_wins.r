...
# Usage: Rscript plot_chr_wins.r -c [chr] -s [chr_size] -o [out_prefix] -n [spp_diffs] -p [pi] -y [dxy] -f [fst] -d [dsuite] -l [ld] -r [recomb] -t [tajimasd] -a [ahmm_list] -e [elai_dose] -i [elai_snpinfo] -g [elai_fam]
...
parser <- add_argument(parser, "-n", help = "Species differences filename", default = "spp_diffs.tab", type = "character")
parser <- add_argument(parser, "-p", help = "Pi filename", default = "all_pi.txt", type = "character")
parser <- add_argument(parser, "-y", help = "Dxy filename", default = "all_dxy.txt", type = "character")
parser <- add_argument(parser, "-f", help = "Fst filename", default = "all_fst.txt", type = "character")
parser <- add_argument(parser, "-d", help = "Dsuite filename", default = "local_Fstats.txt", type = "character")
parser <- add_argument(parser, "-l", help = "LD filename", default = "ld_windows.txt", type = "character")
parser <- add_argument(parser, "-r", help = "Recombination filename", default = "recomb_windows.txt", type = "character")
parser <- add_argument(parser, "-t", help = "Tajima's D filename", default = "snps.TajimaD", type = "character")
