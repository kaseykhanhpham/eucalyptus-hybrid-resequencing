...
stats_df = pd.DataFrame(data = None, columns = ["chr", "start", "end", "Dxy"])
...
# rearrange structure json files so E. cordata is an ingroup and reference E. globulus is the outgroup
struct_json_cord = {"cluster1":{"ecord":outgr_json, "eglob_mr":struct_json["cluster1"]["eglob_mr"]}}
outgr_json_glob = struct_json["cluster1"]["eglob_ref"]
pop_struct = egglib.struct_from_dict(struct_json_cord, outgr_json_glob)

# create ComputeStats instance for Dxy
computer = egglib.stats.ComputeStats(struct = pop_struct, multi_hits = True)
computer.add_stats("Dxy")
...
for chr in chr_list:
    ...
    for window in slider:
        ...
        # save to dataframe
        stats_df.loc[len(stats_df.index)] = [chrom, start, end, dxy]

# export dataframe as tab-delimited text file
stats_df.to_csv("{INFILE}_dxy_cord.tab".format(INFILE = vcf_name), sep = "\t", index = False)
