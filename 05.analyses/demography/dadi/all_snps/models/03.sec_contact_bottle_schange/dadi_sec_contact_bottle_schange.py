'''
Usage: python dadi_sec_contact_bottle_schange.py
...
# 03. Divergence in isolation, instantaneous size change, asymmetric secondary contact with exponential size change
for i in range(1,6):
   Optimize_Functions.Optimize_Routine(fs, pts, "{PREFIX}_run{NUM}".format(PREFIX = prefix, NUM = i), "sec_contact_bottle_schange", all_snps_models.sec_contact_bottle_schange, rounds, 10, fs_folded=fs_folded, reps=reps, maxiters=maxiters, folds=folds, param_labels = "nu1i, nu2i, nu1m, nu2m, nu1f, nu2f, m12, m21, T1, T2")
