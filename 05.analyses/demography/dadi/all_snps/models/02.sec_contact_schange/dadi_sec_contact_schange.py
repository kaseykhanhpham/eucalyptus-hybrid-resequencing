'''
Usage: python dadi_sec_contact_schange.py
For running param optimization for demographic model of divergence in isolation,
followed by a period of asymmetric migration and gradual exponential size change.
...
#**************
snps = "/blue/soltis/kasey.pham/euc_hyb_reseq/call_snps/04.filter_snps/all_fil_biallelic.vcf.gz"
...
# 02. Divergence in isolation, asymmetric secondary contact with exponential size change
for i in range(1,6):
   Optimize_Functions.Optimize_Routine(fs, pts, "{PREFIX}_run{NUM}".format(PREFIX = prefix, NUM = i), "sec_contact_schange", all_snps_models.sec_contact_schange, rounds, 8, fs_folded=fs_folded, reps=reps, maxiters=maxiters, folds=folds, param_labels = "nu1i, nu2i, nu1f, nu2f, m12, m21, T1, T2")
