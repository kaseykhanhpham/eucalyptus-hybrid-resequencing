'''
Usage: python dadi_bottlegrowth.py
For running param optimization for demographic model for bottleneck plus
...
# 04. BottleGrowth
for i in range(1,6):
   Optimize_Functions.Optimize_Routine(fs, pts, "{PREFIX}_run{NUM}".format(PREFIX = prefix, NUM = i), "bottlegrowth", dadi.Demographics1D.bottlegrowth, rounds, 3, fs_folded=fs_folded, reps=reps, maxiters=maxiters, folds=folds, param_labels = "nuB, nuF, T")
