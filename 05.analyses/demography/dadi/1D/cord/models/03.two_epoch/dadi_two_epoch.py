'''
Usage: python dadi_two_epoch.py
For running param optimization for demographic model for bottleneck
...
# 03. Two Epoch
for i in range(1,6):
   Optimize_Functions.Optimize_Routine(fs, pts, "{PREFIX}_run{NUM}".format(PREFIX = prefix, NUM = i), "two_epoch", dadi.Demographics1D.two_epoch, rounds, 2, fs_folded=fs_folded, reps=reps, maxiters=maxiters, folds=folds, param_labels = "nu, T")
