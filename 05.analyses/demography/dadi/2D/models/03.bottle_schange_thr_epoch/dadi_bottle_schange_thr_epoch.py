'''
Usage: python dadi_bottle_schange_thr_epoch.py
For running param optimization for demographic model divergence with an instantaneous
size change and a period of neutral evolution, followed by a period of gradual 
...
# add location of model functions
...
# 03. Divergence, instantaneous size change, gradual exponential size change
for i in range(1,6):
   Optimize_Functions.Optimize_Routine(fs, pts, "{PREFIX}_run{NUM}".format(PREFIX = prefix, NUM = i), "bottle_schange_thr_epoch", all_snps_models.bottle_schange_thr_epoch, rounds, 9, fs_folded=fs_folded, reps=reps, maxiters=maxiters, folds=folds, param_labels = "nu1i, nu2i, nu1m, nu2m, nu1f, nu2f, T1, T2, T3")
