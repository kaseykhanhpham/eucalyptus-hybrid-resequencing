'''
Usage: python dadi_sec_contact_asym_mig_size.py
For running param optimization for demographic model divergence in isolation, followed by
continuous asymmetrical secondary contact with instantaneous size change.
...
# 12. Split with no gene flow, followed by instantaneous size change with continuous asymmetrical migration.
for i in range(2,6):
   Optimize_Functions.Optimize_Routine(fs, pts, "{PREFIX}_run{NUM}".format(PREFIX = prefix, NUM = i), "sec_contact_asym_mig_size", Models_2D.sec_contact_asym_mig_size, rounds, 8, fs_folded=fs_folded, reps=reps, maxiters=maxiters, folds=folds, param_labels = "nu1a, nu2a, nu1b, nu2b, m12, m21, T1, T2")
