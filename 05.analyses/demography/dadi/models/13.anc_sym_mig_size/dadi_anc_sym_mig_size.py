'''
Usage: python dadi_anc_sym_mig_size.py
For running parameter optimization for the demographic model divergence with ancient
continuous symmetrical migration, followed by isolation with instantaneous size change.
...
# 13. Split with continuous symmetrical gene flow, followed by instantaneous size change with no migration.
for i in range(2,6):
   Optimize_Functions.Optimize_Routine(fs, pts, "{PREFIX}_run{NUM}".format(PREFIX = prefix, NUM = i), "anc_sym_mig_size", Models_2D.anc_sym_mig_size, rounds, 7, fs_folded=fs_folded, reps=reps, maxiters=maxiters, folds=folds, param_labels = "nu1a, nu2a, nu1b, nu2b, m, T1, T2")