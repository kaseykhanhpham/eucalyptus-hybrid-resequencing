'''
Usage: python dadi_sym_mig.py
For running parameter optimization for divergence with symmetric migration
...
# 02. Split into two populations, with continuous symmetric migration.
for i in range(2,6):
   Optimize_Functions.Optimize_Routine(fs, pts, "{PREFIX}_run{NUM}".format(PREFIX = prefix, NUM = i), "sym_mig", Models_2D.sym_mig, rounds, 4, fs_folded=fs_folded, reps=reps, maxiters=maxiters, folds=folds, param_labels = "nu1, nu2, m, T")