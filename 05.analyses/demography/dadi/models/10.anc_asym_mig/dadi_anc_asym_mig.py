'''
Usage: python dadi_anc_asym_mig.py
For running parameter optimization for divergence with ancient continuous asymmetrical
migration followed by isolation in dadi.
...
# 10. Split with continuous asymmetric migration, followed by isolation.
# for i in range(2,6):
for i in range(4,6):
    Optimize_Functions.Optimize_Routine(fs, pts, "{PREFIX}_run{NUM}".format(PREFIX = prefix, NUM = i), "anc_asym_mig", Models_2D.anc_asym_mig, rounds, 6, fs_folded=fs_folded, reps=reps, maxiters=maxiters, folds=folds, param_labels = "nu1, nu2, m12, m21, T1, T2")
