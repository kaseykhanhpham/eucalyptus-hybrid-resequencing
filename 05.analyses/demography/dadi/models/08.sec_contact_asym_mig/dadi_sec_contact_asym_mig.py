'''
Usage: python dadi_sec_contact_asym_mig.py
For running parameter optimization for the model divergence in isolation followed
by continuous asymmetric secondary contact.
...
#**************
snps = "/blue/soltis/kasey.pham/euc_hyb_reseq/call_snps/04.filter_snps/all_fil_biallelic.prune.vcf.gz"
...
# 08. Split with no gene flow, followed by period of continuous asymmetrical gene flow.
#for i in range(2,6):
#    Optimize_Functions.Optimize_Routine(fs, pts, "{PREFIX}_run{NUM}".format(PREFIX = prefix, NUM = i), "sec_contact_asym_mig", Models_2D.sec_contact_asym_mig, rounds, 6, fs_folded=fs_folded, reps=reps, maxiters=maxiters, folds=folds, param_labels = "nu1, nu2, m12, m21, T1, T2")

Optimize_Functions.Optimize_Routine(fs, pts, "{PREFIX}_run5".format(PREFIX = prefix), "sec_contact_asym_mig", Models_2D.sec_contact_asym_mig, rounds, 6, fs_folded=fs_folded, reps=reps, maxiters=maxiters, folds=folds, param_labels = "nu1, nu2, m12, m21, T1, T2")
# Needed to re-do run 5 because of an HPG error