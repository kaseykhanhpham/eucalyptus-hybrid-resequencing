'''
Usage: python dadi_asym_mig.py
For running parameter optimization for divergence with continuous asymmetric migration in dadi
...
# 03. Split into two populations, with continuous asymmetric migration.
for i in range(2,6):
    Optimize_Functions.Optimize_Routine(fs, pts, "{PREFIX}_run{NUM}".format(PREFIX = prefix, NUM = i), "asym_mig", Models_2D.asym_mig, rounds, 5, fs_folded=fs_folded, reps=reps, maxiters=maxiters, folds=folds, param_labels = "nu1, nu2, m12, m21, T")