'''
Usage: python dadi_sec_contact_sym_mig_three_epoch.py
Paramater optimization for the demographic model divergence in isolation, continuous
symmetrical secondary contact, followed by isolation.
...
for i in range(1,6):
    Optimize_Functions.Optimize_Routine(fs, pts, "{PREFIX}_run{NUM}".format(PREFIX = prefix, NUM = i), "sec_contact_sym_mig_three_epoch", Models_2D.sec_contact_sym_mig_three_epoch, rounds, 6, fs_folded=fs_folded, reps=reps, maxiters=maxiters, folds=folds, param_labels = "nu1, nu2, m, T1, T2, T3")