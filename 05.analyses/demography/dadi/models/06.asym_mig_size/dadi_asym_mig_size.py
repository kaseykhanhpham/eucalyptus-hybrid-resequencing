'''
Usage: python dadi_asym_mig_size.py
For running parameter optimization for the demographic model divergence with
continuous asymmetric migration and instantaneous size change in dadi.
...
# 06. Split with different migration rates, then instantaneous size change with continuous asymmetric migration.
for i in range(2,6):
    Optimize_Functions.Optimize_Routine(fs, pts, "{PREFIX}_run{NUM}".format(PREFIX = prefix, NUM = i), "asym_mig_size", Models_2D.asym_mig_size, rounds, 8, fs_folded=fs_folded, reps=reps, maxiters=maxiters, folds=folds, param_labels = "nu1a, nu2a, nu1b, nu2b, m12, m21, T1, T2")
