'''
Usage: python dadi_sym_mig_size.py
For running parameter optimization on the model divergence with continuous
symmetric migration and instantaneous size change in dadi.
...
# 05. Split with symmetric migration, then instantaneous size change with continuous symmetric migration.
for i in range(2,6):
    Optimize_Functions.Optimize_Routine(fs, pts, "{PREFIX}_run{NUM}".format(PREFIX = prefix, NUM = i), "sym_mig_size", Models_2D.sym_mig_size, rounds, 7, fs_folded=fs_folded, reps=reps, maxiters=maxiters, folds=folds, param_labels = "nu1a, nu2a, nu1b, nu2b, m, T1, T2")