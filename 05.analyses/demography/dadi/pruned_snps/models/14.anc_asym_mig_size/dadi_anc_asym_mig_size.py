'''
Usage: python dadi_anc_asym_mig_size.py
For running parameter optimization on demographic model for divergence with ancient
continuous asymmetric migration, folowed by isolation with instantaneous size change.
...
# 14. Split with continuous asymmetrical gene flow, followed by instantaneous size change with no migration.
for i in range(2,6):
    Optimize_Functions.Optimize_Routine(fs, pts, "{PREFIX}_run{NUM}".format(PREFIX = prefix, NUM = i), "anc_asym_mig_size", Models_2D.anc_asym_mig_size, rounds, 8, fs_folded=fs_folded, reps=reps, maxiters=maxiters, folds=folds, param_labels = "nu1a, nu2a, nu1b, nu2b, m12, m21, T1, T2")
