'''
Usage: python dadi_anc_sym_mig.py
For running parameter optimization for divergence with ancient continuous
symmetrical migration followed by isolation in dadi.
...
# 09. Split with continuous symmetric migration, followed by isolation.
#for i in range(2,6):
for i in range(4,6):
   Optimize_Functions.Optimize_Routine(fs, pts, "{PREFIX}_run{NUM}".format(PREFIX = prefix, NUM = i), "anc_sym_mig", Models_2D.anc_sym_mig, rounds, 5, fs_folded=fs_folded, reps=reps, maxiters=maxiters, folds=folds, param_labels = "nu1, nu2, m, T1, T2")
# Needed to re-do runs 4 and 5 because of an HPG error