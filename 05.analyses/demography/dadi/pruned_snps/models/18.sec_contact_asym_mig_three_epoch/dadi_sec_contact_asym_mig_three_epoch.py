'''
Usage: python dadi_sec_contact_asym_mig_three_epoch.py
Parameter optimization for the demographic model divergence in isolation, followed
by continuous asymmetrical secondary contact, followed by isolation.
...
The following 6 models were added to the Diversification Model Set by:
    ...
    doi: 10.1111/jbi.13365
'''
# 18. Split with no gene flow, followed by period of continuous asymmetrical migration, then isolation.
for i in range(1,6):
    Optimize_Functions.Optimize_Routine(fs, pts, "{PREFIX}_run{NUM}".format(PREFIX = prefix, NUM = i), "sec_contact_asym_mig_three_epoch", Models_2D.sec_contact_asym_mig_three_epoch, rounds, 7, fs_folded=fs_folded, reps=reps, maxiters=maxiters, folds=folds, param_labels = "nu1, nu2, m12, m21, T1, T2, T3")
