'''
Usage: python dadi_sec_contact_sym_mig.py
For running parameter optimization for the model secondary contact with continuous
symmetric migration in dadi.
...
# 07. Split with no gene flow, followed by period of continuous symmetrical gene flow.
# for i in range(2,6):
#     Optimize_Functions.Optimize_Routine(fs, pts, "{PREFIX}_run{NUM}".format(PREFIX = prefix, NUM = i), "sec_contact_sym_mig", Models_2D.sec_contact_sym_mig, rounds, 5, fs_folded=fs_folded, reps=reps, maxiters=maxiters, folds=folds, param_labels = "nu1, nu2, m, T1, T2")

Optimize_Functions.Optimize_Routine(fs, pts, "{PREFIX}_run5".format(PREFIX = prefix), "sec_contact_sym_mig", Models_2D.sec_contact_sym_mig, rounds, 5, fs_folded=fs_folded, reps=reps, maxiters=maxiters, folds=folds, param_labels = "nu1, nu2, m, T1, T2")
