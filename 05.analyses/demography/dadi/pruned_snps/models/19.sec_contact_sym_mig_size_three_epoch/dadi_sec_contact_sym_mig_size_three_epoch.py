'''
Usage: python dadi_sec_contact_sym_mig_size_three_epoch.py
Parameter optimization for demographic model divergence in isolation, followed by
continuous symmetrical secondary contact with instantaneous size change, isolation.
...
# 19. Split with no gene flow, followed by instantaneous size change with continuous symmetrical migration, then isolation.
for i in range(1,6):
    Optimize_Functions.Optimize_Routine(fs, pts, "{PREFIX}_run{NUM}".format(PREFIX = prefix, NUM = i), "sec_contact_sym_mig_size_three_epoch", Models_2D.sec_contact_sym_mig_size_three_epoch, rounds, 8, fs_folded=fs_folded, reps=reps, maxiters=maxiters, folds=folds, param_labels = "nu1a, nu2a, nu1b, nu2b, m, T1, T2, T3")
