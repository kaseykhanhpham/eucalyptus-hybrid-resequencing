'''
Usage: python dadi_no_mig_size.py
For running parameter optimization on model for divergence with no migration and
instantaneous size change in dadi.
...
# 04. Split with no migration, then instantaneous size change with no migration.
for i in range(2,6):
    Optimize_Functions.Optimize_Routine(fs, pts, "{PREFIX}_run{NUM}".format(PREFIX = prefix, NUM = i), "no_mig_size", Models_2D.no_mig_size, rounds, 6, fs_folded=fs_folded, reps=reps, maxiters=maxiters, folds=folds, param_labels = "nu1a, nu2a, nu1b, nu2b, T1, T2")