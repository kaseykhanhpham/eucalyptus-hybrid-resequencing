...
populate_mat <- function(infile_loc, sample_vec, window_tab){
    mat_list <- list()
    index = 1
...
populate_mat_cutoff <- function(infile_loc, sample_vec, window_tab, cutoff){
    mat_list <- list()
    index = 1
    for(i in c(1:nrow(window_tab))) {
        wchr <- window_tab[i, "chrom"]
        ...
        for(i in c(1:length(sample_vec))){
            ...
            mat_list[[mat_ind]]["window"] <- windex
            mat_list[[mat_ind]]["sample"] <- sample_vec[i]
            mat_list[[mat_ind]]["dosage"] <- dose_cutoff
