...
# genetic_map: a genetic map with the columns chr, variant ID, genetic
#              distance, physical distance in that order.
# out_name: name of the output file to be written with distances corresponding
#           to each row of the provided counts table
...
counts_tab <- read.table(counts_filename, skip = 1, header = FALSE,
                         col.names = c("Chr", "Pos", "nalleles",
                                       "totalalleles", "A_count", "a_count"),
                         colClasses = c("character", "numeric", "numeric",
                                        "numeric", "character", "character"),
                         sep = "\t", fill = TRUE)
map_tab <- read.table(map_filename, header = FALSE, 
                      col.names = c("Chr", "ID", "cM", "bp"),
                      colClasses = c("character", "character",
                                     "numeric", "numeric"))

# define function to interpolate genetic distance if passed a chromosome,
# position, and genetic map.
# Debug test cases:
...
#  write(paste("doing", chr, pos, sep = " "), stdout()) DEBUG
  ...
  if (length(smaller) == 0) {
    ...
    # if adjacent map markers have same genetic dist
    # (i.e., rate of cM per bp will be 0), walk forwards until a marker with
    # a different distance is found
    step_counter = 3
    ...
    # if adjacent map markers have same genetic dist 
    # (i.e., rate of cM per bp will be 0), walk backwards until a marker with
    # a different distance is found
...
absolute_dists <- apply(counts_tab, 1,
                        function(curr_row) get_dist(curr_row[1],
                        as.numeric(curr_row[2]), map_tab)
                        )
rel_dists <- sapply(c(2:length(absolute_dists)), function(x)
                      absolute_dists[x] - absolute_dists[x - 1])
rel_dists <- c(0, rel_dists)

out_tab <- data.frame(cbind(counts_tab$Chr, counts_tab$Pos, rel_dists))

write.table(out_tab, out_name, quote = FALSE, row.names = FALSE,
            col.names = FALSE)